...
namespace Private {
  ...
  bool before_mono_string(winhook::hook_stack *s)
  {
    ...
    if (isBadText((wchar_t*)(p->chars), p->length)
        //|| addresses_.find(p) != addresses_.end()
        || hashes_.find(Engine::hashWCharArray((wchar_t*)(p->chars), p->length)) != hashes_.end())
      return true;
    ...
    std::wstring oldText = std::wstring((wchar_t*)(p->chars) , p->length);
    std::wstring newText = EngineController::instance()->dispatchTextWSTD(oldText, role, split);
    if (!newText.empty() && newText != oldText) {
      if (newText.size() > oldText.size())
        newText = newText.substr(0,oldText.size());
      else {
        hashes_.insert(Engine::hashWStringSTD(newText));
        while (newText.size() < oldText.size())
          newText.push_back(L' ');
      }
      //newText.toWCharArray(p->chars);
      memcpy(p->chars, newText.c_str(), newText.size());
      //addresses_.insert(p);
      hashes_.insert(Engine::hashWCharArray((wchar_t*)(p->chars), p->length));
    }
    return true;
  }
  ...
  void guessSplits()
  {
    if (Engine::globs(L"CM3D*.exe")) { // Custom Maid 3D 2
      scenarioSplit_ = 0x3;

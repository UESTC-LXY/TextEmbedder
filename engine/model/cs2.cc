...
// Remove furigana in scenario thread.
#include<regex>
std::wstring CatSystemEngine::rubyRemove(const std::wstring& text)
{
...
std::wstring CatSystemEngine::translationFilter(const std::wstring& text, int role)
{
    // http://sakuradite.com/topic/1100
    // Replace certain thin characters to wide ones for choice texts
    if (role != Engine::ChoiceRole)
    ...
    if (ret.find('/') != std::wstring::npos) {
        strReplace(ret, L" / ", L"/");
        strReplace(ret, L"/", L"\xff0f");
    } 
    strReplace(ret, L" ", L"\u3000");
    return ret;
}

...
namespace Private {
  ...
  {  
      ...
      std::string  newData = EngineController::instance()->dispatchTextASTD(text, role, sig);
      if (needdollar) {
          newData.append("$");
      }
      if (namesplit) {
          std::string name;
          name=std::string(origin, namesplit); 
          name.append("$&");
          name.append(newData);
          newData = name;
      }
      //MessageBoxA(0, newData.data(), "2", 0);
...
namespace Private {
  ...
  bool hookBefore(winhook::hook_stack *s)
  {
    ...
    strcpy((char*)s->stack[0], data_.c_str());
    //s->stack[0] = (ULONG)data_.c_str();
    return true;
  }

...
namespace Private {
  ...
  bool hookBefore(winhook::hook_stack *s)
  {
    ...
    if (trimmedText != arg->text)
      newData.insert(0,std::string(arg->text, trimmedText - arg->text));
    data_ = newData;
...
std::wstring EscudeEngine::rubyRemove(const std::wstring& text)
{
    if (text.find(L"<ruby") == std::wstring::npos)
        return text;

    static std::wregex rx(L"<ruby.*?>(.*?)</ruby>");

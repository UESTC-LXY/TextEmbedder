...
namespace NameHook { // scenario with name, but scenario are rendered line-by-line and not good place to hook
  std::unordered_set<std::string> texts_;
...
namespace Private {
  ...
  bool hookBefore(winhook::hook_stack *s)
  {
    ...
    std::string oldData(args->begin, args->end - args->begin);
    if (NameHook::texts_.find(oldData)!= NameHook::texts_.end()) // avoid re-translate character name
      return true;
    ...
    std::string newData = EngineController::instance()->dispatchTextASTD(oldData, role, sig);
    if (newData == oldData)
      return true;
    //if (text != args->begin)
    //  newData.prepend(args->begin, text - args->begin);
    data_ = newData;
    args->begin = data_.c_str();
...
//}
#include<regex>
std::wstring DebonosuEngine::rubyRemove(const std::wstring& text)
{
    if (text.find('{') == std::wstring::npos)
        return text;
    static std::wregex rx(LR"(\{(.+?)/.+\})");

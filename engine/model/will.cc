...
class TextHookW
{
  ...
  bool hookBefore(winhook::hook_stack *s)
  {
    ...
    std::wstring oldText = std::wstring(trimmedText, trimmedSize),
            newText = EngineController::instance()->dispatchTextWSTD(oldText, role_, sig);
    if (newText == oldText)
      return true;
    ...
    text_ = newText;
    s->stack[stackIndex_] = (ULONG)text_.c_str();
    return true;
  }
...
namespace Private {
  ...
  bool hookBefore(winhook::hook_stack *s)
  {
    ...
    if (data_.empty()) // do not allow delete other text
      return true;
    s->stack[8] = (ULONG)data_.c_str(); // arg8
    return true;
  }
...
std::wstring WillPlusEngine::rubyRemove(const std::wstring& text)
{

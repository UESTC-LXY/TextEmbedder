...
class TextHook
{
  ...
  bool hookBefore(winhook::hook_stack *s)
  {
   ...
   // s->stack[1] = (ULONG)data_.constData(); // reset arg1
    data_STD = EngineController::instance()->dispatchTextASTD(text, role_, sig);
    s->stack[1] = (ULONG)data_STD.c_str(); // reset arg1
    return true;
  }
...
bool attachScenarioHook(ULONG startAddress, ULONG stopAddress)
{
  ...
  return h->attach(bytes, sizeof(bytes), startAddress, stopAddress)|| h->attach(bytes_kunado_kukoki, sizeof(bytes), startAddress, stopAddress);
}
...
bool attachHistoryHook(ULONG startAddress, ULONG stopAddress)
{
  ...
  static auto h = new TextHook(Engine::OtherRole); // never deleted
...
bool CMVSEngine::attach()
{
  ...
  if (!::attachScenarioHook(startAddress, stopAddress)){
    DOUT("cannot attachScenarioHook");
    return false;
  }

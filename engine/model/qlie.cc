...
//}
#include<regex>
std::wstring QLiEEngine::textFilter(const std::wstring& text, int role)
{
    // 忽略role参数，因为在这个实现中没有用到
    std::wregex rx(L"\\[.+?,.+?\\]");
    return std::regex_replace(text, rx, L"");
}
/**
...
std::wstring QLiEEngine::translationFilter(const std::wstring&text, int role)
{ 
  if (text.empty() || (text.find(L'[')==text.npos) && (text.find(L']') == text.npos))
    return text;
  std::wstring ret = text;
  for (int i = 0; i < ret.size(); i++)
    if (ret[i] == L'[' &&
        (i == ret.size() - 1 || ret[i+1]  < L'a' || ret[i+1]  > L'z'))
      ret[i] = L'(';
  for (int i = 0; i < ret.size(); i++)
    if (ret[i]  == L']' &&
        (i == 0 || ret[i-1]  < L'a' || ret[i-1]  > L'z'))
      ret[i] = L')';
  return ret;
}
...
//}
#include<regex>
std::wstring QLiEEngine::rubyRemove(const std::wstring& text)
{
    if (text.find(L"[rb") == std::wstring::npos)
        return text;
    static std::wregex rx(L"\\[rb,(.+?),.+\\]");

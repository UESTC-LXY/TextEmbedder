...
namespace Private {
  ...
  bool hookBefore(ULONG retaddr, int role, winhook::hook_stack *s)
  {
...
bool attach(ULONG startAddress, ULONG stopAddress)
{
  ...
  auto fun = [&count](ULONG addr) -> bool {
    ...
    case 0x01fc45c6: // 013BF01C   C645 FC 01       MOV BYTE PTR SS:[EBP-0x4],0x1
      if (*(DWORD *)(retaddr - 5 - sizeof(DWORD)) == 0x00D845C6) { // previous instruction
        role = Engine::ScenarioRole;
        break;
      }
    default: return true;
    }
    auto before = std::bind(Private::hookBefore, retaddr, role, std::placeholders::_1);
...
/**
 *  Examples:
 *  まさか、いきなり能{・}力{・}が消えたのかとも思ったけれど、そんなことはなかったらしい。
 *
 * 「あ、ああ。宮{みや}坂{さか}。宮坂終{しゅう}。１年６組だよ」
 */
//std::wstring MinoriEngine::rubyCreate(const std::wstring &rb, const std::wstring &rt)
//{
//  static std::wstring fmt = "%1{%2}";
//  return fmt.arg(rb, rt);
//}

// Remove furigana in scenario thread.
#include<regex>
std::wstring MinoriEngine::rubyRemove(const std::wstring &text)
{
    if (text.find('}') == std::wstring::npos)
        return text;
    static std::wregex rx(L"\\{.*?\\}");
    return std::regex_replace(text, rx, std::wstring());

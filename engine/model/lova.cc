...
namespace Private {
  ...
  bool hookBefore(winhook::hook_stack *s)
  {
    ...
    std::wstring oldText = StringToWideString(std::string(text, size), CP_UTF8);//  QString::fromUtf8(text, size);
    std::wstring _o = oldText;
    strReplace(_o, L"<br>", L"");
    ...
    if (retaddr == returnAddress_) {
      ...
      if (size > 0 && !Util::allAscii(text)) {
        ...
        if (!newText.isEmpty() && newText != oldText) {
          std::string data = newText.toUtf8();

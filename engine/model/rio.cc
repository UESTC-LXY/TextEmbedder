...
std::wstring getRioIni()
{
    std::wstring ret = L"RIO.INI";
  if (Engine::exists(ret))
    return ret;
 ...
 */
int getRioVersion(const std::wstring &path)
{
  std::string line;
  { // get first line
    const wchar_t *ws = (const wchar_t *)path.c_str();
    std::ifstream f(ws);
    if (f.is_open()) {
      std::getline(f, line);
      f.close();
    ...
    void dispatchTextList(LPSTR text)
    {
      ...
      for (auto p = text; p; p = nextText(p)) {
        ...
        if (newData != oldData) {
          ...
          hashes_.insert(Engine::hashByteArraySTD(newData));
        }
      }
    }
...
#include<regex>
std::wstring ShinaRioEngine::rubyRemove(const std::wstring& text)
{
    if (text.find(L"_t!") == std::wstring::npos)
        return text;
    static std::wregex rx(L"_t!.*?[/>]");

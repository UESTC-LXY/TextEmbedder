...
std::wstring LeafEngine::textFilter(const std::wstring &text, int role)
{
    if (role == Engine::ScenarioRole && (text.find('\\') != text.npos)) {
        strReplace(text, L"\\k", L"");
        return text;
  } 
  return text;
}
 ...
 *  Example: <R空港|ここ>
 */
//std::wstring LeafEngine::rubyCreate(const std::wstring &rb, const std::wstring &rt)
//{
//  static std::wstring fmt = "<R%1|%2>";
//  return fmt.arg(rb, rt);
//}

//// Remove furigana in scenario thread.
//std::wstring LeafEngine::rubyRemove(const std::wstring &text)
//{
//  if (!text.contains('<'))
//    return text;
//  static QRegExp rx("<R(.+)\\|.+>");
//  if (!rx.isMinimal())
//    rx.setMinimal(true);
//  return std::wstring(text).replace(rx, "\\1");
//}
#include<regex>
std::wstring LeafEngine::rubyRemove(const std::wstring& text)
{

...
namespace Private {
  ...
  bool hookBefore(winhook::hook_stack *s)
  {
    ...
    data_ = newData;
    s->stack[2] = (DWORD)data_.c_str(); // reset arg2
    return true;
  }
...
namespace Private {
  ...
  bool hookBefore(winhook::hook_stack *s)
  {
    ...
    auto sig = Engine::hashThreadSignature(role, retaddr);
    std::string data = EngineController::instance()->dispatchTextASTD(text, role, sig);
...
//{
//  static QString fmt = QString::fromUtf16(L"\xff5b%2\xff0f%1\xff5d");
...
//    return text;
//  static QRegExp rx(QString::fromUtf16(L"\xff5b.+\xff0f(.+)\xff5d"));
//  if (!rx.isMinimal())
//    rx.setMinimal(true);
//  return QString(text).replace(rx, "\\1");
//}

std::wstring CircusEngine::rubyRemove(const std::wstring& text)
{
    if (text.find(L'\uff5b') == std::wstring::npos)
        return text;
    static std::wregex rx(L"\xff5b.+\xff0f(.+?)\xff5d");

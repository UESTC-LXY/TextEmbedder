...
namespace Private {
  ...
  bool hookBefore(winhook::hook_stack *s)
  {
      if (type_ == Type_BGI3) {
          ...
          auto data_ = EngineController::instance()->dispatchTextASTD((LPCSTR)s->stack[textIndex_], role, sig);
          s->stack[textIndex_] = (DWORD)data_.c_str(); 
          return true;
      }
 ...
 */
//std::string BGIEngine::rubyCreate(const std::string&rb, const std::string&rt)
//{
...
#include<regex>
std::wstring BGIEngine::rubyRemove(const std::wstring& text)
{
    // It could be either <R..> or <r..>
    static const std::wregex rx(L"<r.+?>(.+?)</r>", std::regex_constants::icase);
    std::wstring result = text;
    result = std::regex_replace(result, rx, L"$1");
    return result;
}

...
namespace Private {
  ...
  {
    for (const auto &dll: Engine::glob(L"System/RGSS3*.dll"))
...
bool getMemoryRange(ULONG *startAddress, ULONG *stopAddress)
{
  ...
  if (module.empty())
    return false;
  DOUT("dll =" << module);
  return Engine::getModuleMemoryRange((LPCWSTR)module.c_str(), startAddress, stopAddress);
}
...
namespace Private {
  ...
  {
    static std::unordered_set<std::wstring> texts_;
    ...
    if (arg && arg->isValid()) { // && (quint8)arg->text[0] > 127) { // skip translate text beginning with ascii character
      std::wstring oldText =StringToWideString(std::string(arg->text, arg->size),CP_UTF8),// QString::fromUtf8(arg->text, arg->size),
              prefix,
              suffix,
      ...
      if (!trimmedText.empty() && (texts_.find(trimmedText)==texts_.end())) { // skip text beginning with ascii character
        ...
        std::wstring newText;
        if (trimmedText.find(L'\n')==trimmedText.npos) {
          auto role = guessTextRole(trimmedText);
          auto sig = Engine::hashThreadSignature(role);
          newText = q->dispatchTextWSTD(trimmedText, role, sig, 0, sendAllowed);
        } else { // handle each line one by one
          std::vector<std::wstring> newTexts; 
          for(const auto &eachOldText: strSplit(trimmedText,L"\n")) {
              std::wstring eachPrefix,
                    eachSuffix,
                    eachTrimmedText = trim(eachOldText, &eachPrefix, &eachSuffix);
            if (eachTrimmedText.empty() || (texts_.find(eachTrimmedText)!=texts_.end()))
              newTexts.push_back(eachOldText);
            else {
              auto role = guessTextRole(eachTrimmedText);
              auto sig = Engine::hashThreadSignature(role);
              std::wstring eachNewText = q->dispatchTextWSTD(eachTrimmedText, role, sig, 0, sendAllowed);
              if (eachNewText == eachTrimmedText)
                ...
                if (!eachPrefix.empty())
                  eachNewText.insert(0,eachPrefix);
                if (!eachSuffix.empty())
                  eachNewText.append(eachSuffix);

                newTexts.push_back(eachNewText);
              }
            }
          }
          newText = strjoin(newTexts, L"\n");// newTexts.join(QChar('\n'));
        }
...
namespace Private {
  ...
  bool hookBefore(winhook::hook_stack *s)
  {
    ...
    if (arg->isValid()) {
      auto oldText =StringToWideString(std::string(arg->text),CP_UTF8);
...
//}
#include<regex>
std::wstring RGSSEngine::textFilter(const std::wstring& text, int role)
{
    if (role == Engine::OtherRole || text.find(L'\\') == std::wstring::npos)
        return text;

    std::wstring ret = text;
    static const std::wregex rx_(L"\\\\[0-9A-Z.\\[\\]]+");
    ret = std::regex_replace(ret, rx_, L"");

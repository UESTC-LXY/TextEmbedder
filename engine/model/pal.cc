...
namespace Private {
  bool hookBefore(winhook::hook_stack *s)
  {
    ...
    s->stack[2] = (ULONG)data_.c_str(); // arg2
    return true;
  }
} // namespace Private
...
std::wstring PalEngine::textFilter(const std::wstring& text, int role) {
    if (text.find(L'<')==text.npos)
        return text;

    std::wstring ret = text;
    ret.erase(ret.find(L"</c>"));

    static std::wregex rx(L"<c.*>");
    ret = std::regex_replace(ret, rx, L"");

    return ret;
}

std::wstring PalEngine::rubyRemove(const std::wstring& text) {
    if (text.find(L"</r>")==text.npos)
        return text;

    static std::wregex rx(L"<r.+>(.+?)</r>");

  ...
  bool hookBefore(winhook::hook_stack *s)
  {
    ...
    if (arg->scenarioFlag == 0) {
      ...
      std::string oldData = text,
                 newData = q->dispatchTextASTD(oldData, role, sig);
      if (!newData.empty() && newData != oldData) {
        nameArg_ = arg;
        ::memcpy(nameText_, oldData.c_str(), min(oldData.size() + 1, MaxNameSize));
        ::memcpy(text, newData.c_str(), min(newData.size() + 1, MaxNameSize));
      }
 ...
 * Format: L"【\1】\2"
 */
std::wstring ElfEngine::textFilter(const std::wstring&text, int role)
{
  ...
  const wchar_t
    ...
    w_colon = 0xff1a;         /* ： */
  if (text.empty() || text[0] != w_angle_open)
    return text;
  int pos = text.find(w_angle_close);
  if (pos == text.npos)
    return text;
  ...
  if (pos + 1 < ret.size() && ret[pos + 1] == w_colon)
      strReplace(ret, std::wstring(pos + 1, 1), L"");
//    ret.remove(pos + 1, 1);
  return ret;
}

...
public:
  ...
  explicit EmbedDriverPrivate(void*parent)
...
EmbedDriver::EmbedDriver(void*parent)
  :   d_(new D(this))
{
...
void EmbedDriver::setEncoding(const std::wstring&v)  { if (d_->engine) d_->engine->setEncoding(v); }
...
void EmbedDriver::setSpacePolicyEncoding(const std::wstring&v) { if (d_->engine) d_->engine->setSpacePolicyEncoding(v); }
...
std::wstring EmbedDriver::engineName() const
{ return d_->engine ? d_->engine->name() : L""; }

void EmbedDriver::sendEngineName()
{ engineNameChanged(engineName()); }
...
//void EmbedDriver::updateTranslation(const QString &text, UINT64 hash, int role)
